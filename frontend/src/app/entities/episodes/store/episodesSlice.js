...
export const getEpisodes = createAsyncThunk(
	'episodesApp/episode/getEpisodes',
	async (queryParams, { getState }) => {
		const uid = queryParams?.uid
		const entity = queryParams?.entity
		const endPoint = (entity && uid) ? '/api/v1/entity/' + entity + '/' + uid + '/episodes/' : url
...
export const updateMultipleEpisodes = createAsyncThunk(
	...
	async ({ multipleEpisodeList, project }, { dispatch, getState }) => {
		const response = await axios.post('/api/v1/entity/project/' + project + '/episode_bulk_update/', multipleEpisodeList);
		const data = await response.data;
