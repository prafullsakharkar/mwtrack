...
import { setUser, updateUserSettings } from '@/stores/userSlice';
...
function getInitialSettings() {
  const defaultLayoutStyle =
    settingsConfig.layout && settingsConfig.layout.style ? settingsConfig.layout.style : 'layout2';
  const layout = {
...
export const setDefaultSettings = createAsyncThunk(
  'core/settings/setDefaultSettings',
...
const getDirection = (state) => state.core.settings.current.direction;
const getMainTheme = (state) => state.core.settings.current.theme.main;
const getNavbarTheme = (state) => state.core.settings.current.theme.navbar;
const getToolbarTheme = (state) => state.core.settings.current.theme.toolbar;
const getFooterTheme = (state) => state.core.settings.current.theme.footer;
...
export const selectCurrentSettings = ({ core }) => core.settings.current;

export const selectCurrentLayoutConfig = ({ core }) => core.settings.current.layout.config;

export const selectDefaultSettings = ({ core }) => core.settings.defaults;

export const selectThemesSettings = ({ core }) => core.settings.themes;
