...
export const getNotes = createAsyncThunk(
	...
	async (queryParams, { getState }) => {
		...
		const endPoint = (entity && uid)
			? '/api/v1/entity/' + entity + '/' + uid + '/notes/'
			: (user)
				? '/api/v1/user/account/' + user + '/notes/'
				: url
...
export const updateMultipleNotes = createAsyncThunk(
	...
	async ({ multipleNoteList, project }, { dispatch, getState }) => {
		const response = await axios.post('/api/v1/entity/project/' + project + '/note_bulk_update/', multipleNoteList);
		const data = await response.data;
		return data;
	}
);
...
export const { selectAll: selectNotes, selectEntities: selectNote, selectById: selectNotesById } = notesAdapter.getSelectors(
	state => state.notesApp ? state.notesApp.notes : state.overviewApp ? state.overviewApp.notes : state.profileApp.notes
);
